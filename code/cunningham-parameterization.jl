...
begin
    fig = Figure(resolution = (600, 300), backgroundcolor = RGBAf(0.0, 0.0, 0.0, 0.0))
    ax1 = Axis(
        fig[1, 1],
        aspect = DataAspect(),
        ylabel = L"\beta",
    ...
    text!(ax1, -13, 10, text = "θ = 75°", fontsize = LARGE_FONT_SIZE, font = :bold)

    Makie.save("presentation/figs/_raw/reparameterization.svg", fig)
    fig
end
