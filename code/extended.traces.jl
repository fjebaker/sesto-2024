...
function calc_emission(m, d, model, rot)
    ...
    difference, index = findmin([abs(left_r[i] - right_r[j]) for i in eachindex(left_r), j in eachindex(right_r)])
...
xs = fill(x0, size(vs))
simsols = tracegeodesics(m, xs, vs, d, 2000.0; save_on = false, ensemble = Gradus.EnsembleEndpointThreads())
...
begin
    ...
    plotring(ax, model.r; horizon_r = R, height = model.h, color = :black, dim = dim, linestyle = :dash, linewidth = 1.0)
    ...
    for sol in right
        plot_sol(ax, sol; color = popfirst!(palette), horizon_r = R, dim = dim, show_intersect = true)
    end
    for sol in left
        plot_sol(ax, sol; color = popfirst!(palette), horizon_r = R, dim = dim, show_intersect = true, linestyle = :dash)
    end
    
    plot_obscured_line!(ax, SVector(0.0, 0.0, 0.0), SVector(model.r, 0.0, model.h); horizon_r = R - 0.2, color = :black, linestyle = :dash, linewidth = 1.0)


    plotring(ax, model.r; horizon_r = R, height = -model.h, color = :black, dim = dim, linewidth = 1.0)
