...
flat_tf1 = ring_transfer_function(Gradus.SphericalMetric(), d, RingCorona(ring_radius_1, 5.0), rs, ts)
flat_tf2 = ring_transfer_function(Gradus.SphericalMetric(), d, RingCorona(ring_radius_2, 5.0), rs, ts)
...
begin
    ...
    heatmap!(ax2, ts, rs, log10.(flat_tf2), colormap = Reverse(:matter), colorrange = crange)
